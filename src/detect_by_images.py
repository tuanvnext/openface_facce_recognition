...
output_dir = os.path.join(fileDir, '..','demos', 'output')

def draw_face(image, bb, name):
    cv2.rectangle(image, (bb.left(), bb.top()), (bb.right(), bb.bottom()), (0, 255, 0), 2)
    y = bb.top() - 15 if bb.top() - 15 > 15 else bb.top() + 15
    cv2.putText(image, name, (bb.left(), y), cv2.FONT_HERSHEY_SIMPLEX,
                0.75, (0, 255, 0), 2)
    return image
...
def infer(args, multiple=False):
    ...
    for img in args.imgs:
        ...
        for r in reps:
            ...
            if confidence < 0.65:
                person = 'Unknow'
            image = draw_face(image, bbx, person.decode('utf8'))
        ...
        path = os.path.join(output_dir, os.path.basename(img))
